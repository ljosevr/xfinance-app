...
public class VentaServiceImpl implements VentaService {
    ...
    public Venta registrarVenta(Usuario usuario, List<PventaDTO> listVenta, Cliente cliente) throws HandledException {
        ...
        listVenta.forEach(pv -> {
            ...
            if (invActualCosto != null && invActualCosto.isManageStock() && invActualCosto.getCantidad().compareTo(cantidad) >= 0) {
            ...
            try {
                ...
                logger.error("Error al Procesar inventario y actualizar");
            }
        });
        ...
        if(listItems.size() > 0) {
            ...
            throw new HandledException("0" , "Error al registrar venta");
        }
        logger.info("<-- Registrar Venta");
        return venta;
    }
    ...
    private PventaDTO getPventaDTO(String filter, Empresa empresa){
        ...
        if(producto != null) {
            ...
            if (inStock.compareTo(BigDecimal.ZERO) > 0 || manageStock) {
                ...
                if (productoValorVenta != null) {
                    result = convertProductoToPventaDTO(producto, productoValorVenta.getValorVenta(), inStock, manageStock);
                } else {
